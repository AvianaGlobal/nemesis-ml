    ...
    for c in df:
        ...
        if (column_type(c, df_type) != 'Flag_Continuous' and column_type(c, df_type) != 'Flag_Categorical'):
            ...
            if (column_type(c, df_type) == 'Continuous') and (get_min(df[c]) == get_max(df[c])):
                ...
                df_type = df_type.drop(index=int(df_type[df_type.Variable == c].index[0]))
                continue
            if (column_type(c, df_type) == 'Ordinal' or column_type(c, df_type) == 'Nominal') and (
                    get_mode(df[c])[1] / get_valid_num(df[c]) > 0.95):
            ...
            if column_type(c, df_type) == 'Continuous':
                ...
                if get_distinct_num(df[c]) > 5:
                    file.write(
                        'Number of distict categories is larger than 5. We stop updating the number of distinct values\n')
                else:
                    dict_count = get_distinct_count(df[c])
                    for k, v in dict_count.items():
                        file.write(str(k) + ' >>> ' + str(v) + '\n')
...
def fill_missing_value(mydata, column_type):
    ...
    for typ in typelist:
        ...
        if (typ == 'Flag_Continuous' or typ == 'Flag_Categorical'):
            ...
            count_median_num = mydata[column_list[i]][
                mydata[column_list[i]] == num_median].count()  # count the the number of cases in the median category
            """print('')
            print('Column:', column_list[i])
            ...
            count_mode = mydata[column_list[i]][
                mydata[column_list[i]] == mode_value].count()  # count the the number of cases in the modal category
            """print('')
            print('Column:', column_list[i])
    ...
    for i in n_columns:
        ...
        if Predictor_type == 'Nominal':
            ...
            if T_colnumber != i:
                ...
                if Flag_type1 == True:
                    New_Categories = Supervised_Merged(file, dataset, Pre_type,
                                                       dependent_variable_name=dep_variable_name, indep_column_num=i,
                                                       Categorical=False)
                else:
                    New_Categories = Supervised_Merged(file, dataset, Pre_type,
                                                       dependent_variable_name=dep_variable_name, indep_column_num=i)
        ...
        if Predictor_type == 'Ordinal':
            ...
            # Check if target is Categorical or Continuous
            if Flag_type1 == True:
                New_Categories = Supervised_Merged(file, dataset, pre_type, dependent_variable_name=dep_variable_name,
                                                   indep_column_num=i, Categorical=False)
            else:
                New_Categories = Supervised_Merged(file, dataset, pre_type, dependent_variable_name=dep_variable_name,
                                                   indep_column_num=i)
...
def main():
    ...
    file = open('../../model_results/build_features/' + data_file_name + '_processed_univariate_statistic_report.txt',
                'w')
